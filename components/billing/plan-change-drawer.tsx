...
export function PlanChangeDrawer({
  ...
  const currentPlan =
    plans.find((plan) => plan.id === preview.currentPlanId) ??
    getPlanForInterval(plans, currentBaseSlug, "month")
  const targetPlan = plans.find((plan) => plan.id === preview.targetPlanId)
  const currentIntervalLabel =
    currentPlan?.billingInterval === "year" ? "/year" : "/month"
  const targetIntervalLabel =
    targetPlan?.billingInterval === "year" ? "/year" : "/month"
  ...
  const amountDue = preview.amountDue
              ...
              <div className="relative overflow-hidden rounded-2xl border border-border bg-card">
                ...
                <div className="border-t border-border/40 px-5 py-2.5">
                  <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    ...
                    Period: {formatDate(preview.periodStart)} →{" "}
                  ...
                  <div className="divide-y divide-border/40">
                    ...
                    <div className="flex items-center justify-between px-5 py-4">
                      ...
                      <span className="text-lg font-extrabold tabular-nums">
                        {formatMoneyCents(preview.total, currency)}
                      </span>
                    </div>
                  </div>
                </div>
