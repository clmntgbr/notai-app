...
import { PlanChangeDrawer } from "@/components/billing/plan-change-drawer"
import { Button } from "@/components/ui/button"
import { Switch } from "@/components/ui/switch"
import { usePlans } from "@/lib/plan/hooks"
import {
  PLAN_META,
  PLAN_ORDER,
  formatPlanPrice,
...
export function Pricing() {
  ...
  const isCreating = createSubscription.isPending

  const activePlanId = subscription?.plan?.id ?? null
  const activeBaseSlug = subscription?.plan?.slug
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      const nextPreview = await previewSubscription.mutateAsync(planId)
  ...
  const handleConfirmPlanChange = async () => {
    if (!previewPlanId || preview?.prorationDate == null) {
      ...
      {isLoading ? (
        ...
        <section className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {PLAN_ORDER.map((slug) => {
            ...
            const isSameTier = activeBaseSlug === slug
            ...
            const isFreeCard = plan.price === 0 || plan.slug === "free"
            const freeDisabled =
            ...
            return (
              ...
              >
                ...
                <Button
                  ...
                  disabled={isCreating || isCurrent || freeDisabled}
                ...
                >
                  {pendingPlanId === plan.id ? (
                    ...
                    annual ? "Switch to annual" : "Switch to monthly"
                  ) : (
                    meta.cta
                  )}
                </Button>
