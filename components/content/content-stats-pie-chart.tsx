...
import { contentResultChartConfig, CONTENT_RESULT_KEYS } from "@/components/content/content-result-chart-config"
...
import { ContentStatsCounts } from "@/lib/content/types"
import { ChartPieIcon } from "lucide-react"
import { Pie, PieChart } from "recharts"

function toPieData(counts: ContentStatsCounts) {
  return CONTENT_RESULT_KEYS.map((key) => ({
    key,
    value: counts[key],
    fill: `var(--color-${key})`,
  }))
...
export interface ContentStatsPieChartProps {
  counts?: ContentStatsCounts | null
  isLoading?: boolean
  isError?: boolean
}

export function ContentStatsPieChart({
  counts,
  isLoading,
  isError,
}: ContentStatsPieChartProps) {
  const chartData = counts ? toPieData(counts) : []
  const total = counts
    ? CONTENT_RESULT_KEYS.reduce((sum, key) => sum + counts[key], 0)
    : 0
