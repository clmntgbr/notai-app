...
export function MediaStatsSection({ campaignId }: MediaStatsSectionProps) {
  ...
  const { data, isPlaceholderData } = useMediaStats({ campaignId, from, to })
  const dateReady = Boolean(range?.from)

  // Reset local range when switching campaign context.
  useEffect(() => {
    setRange(undefined)
  ...
  useEffect(() => {
    if (userControlled || isPlaceholderData || !data?.from) return
    setRange({
      from: parseApiDate(data.from),
      to: data.to ? parseApiDate(data.to) : undefined,
    })
  }, [data?.from, data?.to, userControlled, isPlaceholderData])
