...
export async function getSubscription(): Promise<Subscription | null> {
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Failed to fetch subscription")
  }
...
export async function previewSubscription(
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Failed to preview subscription")
  }
...
export async function createBillingPortalSession(): Promise<BillingPortalResponse> {
  ...
  if (!response.ok) {
    throw await parseApiError(response, "Failed to open billing portal")
  }
